/****************************************************************************
** Form implementation generated from reading ui file 'choosedialog.ui'
**
** Created: Wed Oct 12 13:53:12 2011
**
** WARNING! All changes made in this file will be lost!
****************************************************************************/

#include "choosedialog.h"

#include <qvariant.h>
#include <qlabel.h>
#include <qcombobox.h>
#include <qpushbutton.h>
#include <qlayout.h>
#include <qtooltip.h>
#include <qwhatsthis.h>
#include <qimage.h>
#include <qpixmap.h>

static const unsigned char image0_data[] = { 
    
};


/*
 *  Constructs a ChooseDialog as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 *
 *  The dialog will by default be modeless, unless you set 'modal' to
 *  TRUE to construct a modal dialog.
 */
ChooseDialog::ChooseDialog( QWidget* parent, const char* name, bool modal, WFlags fl )
    : QDialog( parent, name, modal, fl )
{
    QImage img;
    img.loadFromData( image0_data, sizeof( image0_data ), "PNG" );
    image0 = img;
    if ( !name )
	setName( "ChooseDialog" );
    setIcon( image0 );
    ChooseDialogLayout = new QGridLayout( this, 1, 1, 11, 6, "ChooseDialogLayout"); 

    layout15 = new QVBoxLayout( 0, 0, 6, "layout15"); 

    chooseLabel = new QLabel( this, "chooseLabel" );
    layout15->addWidget( chooseLabel );

    chooseCombo = new QComboBox( FALSE, this, "chooseCombo" );
    chooseCombo->setMinimumSize( QSize( 210, 0 ) );
    layout15->addWidget( chooseCombo );

    ChooseDialogLayout->addLayout( layout15, 0, 0 );

    layout17 = new QHBoxLayout( 0, 0, 6, "layout17"); 
    spacer9 = new QSpacerItem( 100, 20, QSizePolicy::Expanding, QSizePolicy::Minimum );
    layout17->addItem( spacer9 );

    buttonOk = new QPushButton( this, "buttonOk" );
    layout17->addWidget( buttonOk );

    buttonCancel = new QPushButton( this, "buttonCancel" );
    layout17->addWidget( buttonCancel );

    ChooseDialogLayout->addLayout( layout17, 1, 0 );
    languageChange();
    resize( QSize(303, 104).expandedTo(minimumSizeHint()) );
    clearWState( WState_Polished );

    // signals and slots connections
    connect( buttonOk, SIGNAL( clicked() ), this, SLOT( ok() ) );
    connect( buttonCancel, SIGNAL( clicked() ), this, SLOT( reject() ) );
}

/*
 *  Destroys the object and frees any allocated resources
 */
ChooseDialog::~ChooseDialog()
{
    // no need to delete child widgets, Qt does it all for us
}

/*
 *  Sets the strings of the subwidgets using the current
 *  language.
 */
void ChooseDialog::languageChange()
{
    setCaption( tr( "Choose" ) );
    chooseLabel->setText( tr( "Choose..." ) );
    buttonOk->setText( tr( "OK" ) );
    buttonCancel->setText( tr( "Cancel" ) );
}

void ChooseDialog::ok()
{
    qWarning( "ChooseDialog::ok(): Not implemented yet" );
}

