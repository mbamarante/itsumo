...
agent_list_t parseAgents(char* fileName, topologyReader *n, int type, int plan, int agentid)
{
	agent_list_t ans;
	...
	for(agent = agents->FirstChildElement("agent");agent; agent = agent->NextSiblingElement("agent")) 
	{
		...
		if (static_cast<string>(VALUE(ty))=="WLU-learning"){
			LocalAgent* la;
			...
			la = new LocalAgent(ids, agentName, agentStat, memSize, states, learning, discount,startTime, type, plan, agentid);			 		
			(la)->setTopology(n);			
			ans.push_back(la);
		}
	}
...
int main ( int argc, char **argv )
{
	...
	if (argc < 5) {
		cout 	<< "start_agents " <<  endl
			<< "Usage: " << endl 
			<< "\t" << argv[0]
			<< " <agents file> <network topology file> <simulation type>  <fixed plan>  <fixed agent id>"
			<< endl;
		exit(0);
	}
	...
	agent_list_t al = parseAgents(argv[1], n, atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
