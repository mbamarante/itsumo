...
public class SensorFieldPanel_AllTargets extends JPanel implements ActionListener {
	 ...
	 * @uml.associationEnd 
	 * @uml.property name="sfg" multiplicity="(1 1)" inverse="sfp:testbed.dbreakout.graphics.SensorFieldGUI_AllTargets"
	 */
	 ...
	 * @uml.associationEnd 
	 * @uml.property name="nodes" multiplicity="(0 -1)" elementType="testbed.dbreakout.CSP.CSPNodeDB"
	 */
	Vector nodes;
	 ...
	 * @uml.associationEnd 
	 * @uml.property name="text_assignmentsCHB" multiplicity="(1 1)"
	 */
	...
	boolean paint_graph_assignments = false;
	...
	public SensorFieldPanel_AllTargets(SensorFieldGUI_AllTargets sfg) {
		...
		/*
		        chkBoxPanel.setBackground(new Color(70,70,70));
		        chkBoxPanel.setForeground(new Color(50,200,50));
		 */
			...
			try {
				...
				for (int i = 0; i < targets.size(); i++) {
					...
					if (paint_text_assignments)
						g.drawString(vars.toString(), getSize().width - 220, i * 20 + 100);
					for (int j = 0; j < vars.size(); j++) {
						CSPVariableDBA var = (CSPVariableDBA) vars.elementAt(j);
						if (var.getValue() != null) {
							if (paint_domains) {
								...
								Sensor si = sa.getSensor(var.getValue().intValue());
					...
					if (paint_csp_graph) {
						...
						for (int j = 0; j < neighbours.size(); j++) {
							CSPNodeDB other = (CSPNodeDB) neighbours.elementAt(j);
				...
				System.out.println("Problem occured while drawing field for " + sfg.myAgent.getMyName());
				e.printStackTrace();
			}
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equalsIgnoreCase("0"))
			...
			System.out.println("Strange command arrived!!!" + cmd);
			return;
		}
		repaint();
	}
