...
public class Adopt{
  ...
  public Adopt(Problem p, Variable vvar, boolean useLimit, boolean useContext,
    ...
    Simulator.ValueVector.addVarToContext(x_i, d_i);		//Change made by R&S
	...
	c.x_l = childVar;
	c.d = val;
	//c.t = childVar.getInitThreshold(Simulator.ValueVector.valueOf(childVar)); //this should not be there
	//c.lb = c.t;
	costs.add(c);
      }
    }
  ...
  public void whenReceivedVALUE(Variable vvar, Value vval, int thresh){
    ...
    if(vvar.equalVar(parentVar)){
        if(thresh > x_i.getInitThreshold()) 
        	THRESHOLD = thresh;
   		else THESHOLD = x_i.getInitThreshold();
...
//     printCurrentState(Utility.MSG_LEVEL2);
  ...
  public void backtrack(){
    ...
    if(iWantToBacktrack){
      ...
      if(LB_di < x_i.getInitThreshold(d_i)) 
      
      	{	//System.out.print("In Var"+x_i.varIDof()+" Before LB_di = "+LB_di); 
      		LB_di = x_i.getInitThreshold(d_i);
      		//System.out.println("  After LB_di  = "+LB_di);
      }
	...
	if(lb_d < x_i.getInitThreshold(d)) 
		{
			//System.out.print("In Var"+x_i.varIDof()+" Before lb_d = "+ lb_d);
			lb_d = x_i.getInitThreshold(d);
			//System.out.println(" After lb_d = "+ lb_d);
		}
   ...
   // if(LB < x_i.getInitThreshold(min_d)) {
	//	  LB = x_i.getInitThreshold(min_d);
		  //THRESHOLD = LB;
	 //}
      ...
      //System.out.println("Minimum LB = "+LB);
      /* error check */
	...
	//System.out.println("var - "+x_i.varIDof()+" value = "+d_i);
...
//				THRESHOLD = x_i.getInitThreshold(d_i);	
  ...
  public int getMyInitialThreshold(Value val) {
	  ...
	  for(int i = 0;i < childrenVar.size();i++) {
		  Variable childVar = (Variable) childrenVar.elementAt(i);
		  Value Childval = Simulator.ValueVector.valueOf(childVar);
		  if(Childval == null )
		  	  	totalChildInitThresh += childVar.getInitThreshold();
		  else
		  		totalChildInitThresh += childVar.getInitThreshold(Childval);
	  }
			...
			if(pbm.compareDeltas(bb, total_t) > 0 ){
				...
				while(pbm.compareDeltas(bb,0) > 0 && pbm.compareDeltas(Problem.MAX_VALUE, bb) > 0 && i < n){
					...
					if(childVal == null )
							...
							threshold = childVar.getInitThreshold(childVal);
   ...
   if(childVal == null)
   {
    ...
    if(pbm.compareDeltas(excess, 0) > 0 ){
	 ...
	 as much as we can */
  	...
  	{
    if(childVar.getInitThreshold(childVal) > c.lb)
        excess = pbm.subDeltas(c.t, childVar.getInitThreshold(childVal));
    else
    	excess = pbm.subDeltas(c.t, c.lb);
    /* does this child have any room to lower it's threshold? */
    if(pbm.compareDeltas(excess, 0) > 0 ){
	 ...
	 as much as we can */
      if(pbm.compareDeltas(amt, excess) >= 0){
		  if(childVar.getInitThreshold(childVal) >= c.lb)
			c.t = childVar.getInitThreshold(childVal);
		  else if(childVar.getInitThreshold(childVal) < c.lb)
			c.t = c.lb;
		  result = pbm.subDeltas(amt, excess);
      }
      /* it's enough, raise it as far as we need to */
...
//     Algorithm.waitForKey(true);
  ...
  public void updateContexts(Variable vvar, Value vval){
    ...
    for(int i=0; i< costs.size();i++){
		...
		//System.out.println("Cost info x_l="+c.x_l.varIDof()+"c.d="+c.d+"d="+d_i.toString());
	...
	if(!c.context.compatible(cvw)){
 	...
 	//System.out.println("Value of"+c.x_l.varIDof()+"="+ Simulator.ValueVector.valueOf(c.x_l).toString());
	...
	if( t1 == null)
	{
		...
		c.lb = c.x_l.getInitThreshold();
 		...
 		c.lb = c.x_l.getInitThreshold(t1);
	...
	for(int i=0; i< costs.size();i++){
	 ...
	 //System.out.println("Value of"+c.x_l.varIDof()+"="+ Simulator.ValueVector.valueOf(c.x_l).toString());
	 Value t2 = Simulator.ValueVector.valueOf(c.x_l);
	 if (t2 == null)
	 	{
	 		System.out.println("this is null");
	 		c.lb = c.x_l.getInitThreshold();
	 	...
	 	c.lb = c.x_l.getInitThreshold(t2);
