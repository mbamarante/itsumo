...
public class ShiftOp extends GenOp
{
    ...
    {
        return applyAt( RandomInt.next(
            compNumGenes( chrom1, len ) - 1 ) * len, chrom1 );
    }
    ...
    public Chrom applyAt( int index, Chrom chrom1 )
    {
        ...
        if (numGenes > 1)
        {
            ...
            if (index <= chrom1.size() - (len * 2))
            {
                long value1 = chrom1.getLongAt( index, len );
                long value2 = chrom1.getLongAt( index + len, len );
                ...
                chrom1.setLongAt( index, len, value1 );
                chrom1.setLongAt( index + len, len, value2 );
            }
        }
