...
simulation leNet::leRede(TiXmlDocument doc){
	...
	for(streetX=streetsX->FirstChildElement("street");streetX;streetX=streetX->NextSiblingElement("street")){
		...
		unique++;
		street_id->FirstChild()->SetValue(itoa(unique,10));
		...
		for(sectionX=sections->FirstChildElement("section");sectionX;sectionX=sectionX->NextSiblingElement("section")){
			...
			for(lanesetX=lanesetsX->FirstChildElement("laneset");lanesetX;lanesetX=lanesetX->NextSiblingElement("laneset")){
				...
				unique++;
				//cout << atoi(VALUE(laneset_id)) << endl;
				u_laneset[atoi(VALUE(laneset_id))] = unique;
				laneset_id->FirstChild()->SetValue( itoa(unique,10) );
				//cout << u_laneset[atoi(VALUE(laneset_id))] << endl;
				...
				for (directions=turning_probabilities->FirstChildElement("direction"); directions; directions=directions->NextSiblingElement("direction")) {
					...
					TiXmlElement *destination_laneset = directions->FirstChildElement("destination_laneset");
					int index_xx = atoi(VALUE(destination_laneset));

					if (!u_laneset[index_xx]) {
						unique++;
						u_laneset[atoi(VALUE(destination_laneset))] = unique;
					}
						
					cout << u_laneset[index_xx] << endl;
					destination_laneset->FirstChild()->SetValue( itoa(u_laneset[index_xx],10) );
				...
				for(lanes=lanesetX->FirstChildElement("lanes");lanes;lanes=lanes->NextSiblingElement("lanes")){
					...
					for(laneX=lanes->FirstChildElement("lane");laneX;laneX=laneX->NextSiblingElement("lane")){
						...
						unique++;
						u_lane[atoi(VALUE(lane_id))] = unique;
						lane_id->FirstChild()->SetValue(itoa(u_lane[atoi(VALUE(lane_id))],10));
	...
	TiXmlElement *traffic_lights=nos->NextSiblingElement("traffic_lights");
	TiXmlElement *traffic_light;


	for(traffic_light=traffic_lights->FirstChildElement("traffic_light"); traffic_light; traffic_light=traffic_light->NextSiblingElement("traffic_light")){

		TiXmlElement *signalplans=traffic_light->FirstChildElement("signalplans");
		TiXmlElement *signalplan;

		for(signalplan=signalplans->FirstChildElement("signalplan"); signalplan; signalplan=signalplan->NextSiblingElement("signalplan")){

			TiXmlElement *phases=signalplan->FirstChildElement("phases");
			TiXmlElement *phase;

			for(phase=phases->FirstChildElement("phase"); phase; phase=phase->NextSiblingElement("phase")){

				//TiXmlElement *phase_id=phase->FirstChildElement("phase_id");
				//cout << VALUE(phase_id) << endl;

				TiXmlElement *directions=phase->FirstChildElement("directions");
				TiXmlElement *direction;

				for(direction=directions->FirstChildElement("direction"); direction; direction=direction->NextSiblingElement("direction")){

					TiXmlElement *from_lane=direction->FirstChildElement("from_lane");
					int index_xx = atoi(VALUE(from_lane));
					from_lane->FirstChild()->SetValue(itoa(u_lane[index_xx],10));

					TiXmlElement *to_laneset=direction->FirstChildElement("to_laneset");
					index_xx = atoi(VALUE(to_laneset));
					to_laneset->FirstChild()->SetValue( itoa(u_laneset[index_xx],10) );
                ...
                <signalplan>
                    ...
                    <phases>
                        <phase>
                            ...
                            <iteration_end>29</iteration_end>
                            <directions>
                                <direction>
                                    <from_lane>231</from_lane> */
