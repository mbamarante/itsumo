...
void window_to_world(GLint x, GLint y, GLfloat *result)
{
...
/*   printf("winz = %f\n", winZ); */
  ...
  zprReferencePoint[0]=posX;
  zprReferencePoint[1]=posY;
  zprReferencePoint[2]=posZ;

  // (bruno) why the hell 'result' was not setted before!?!
  result[0] = posX;
  result[1] = posY;
  result[2] = posZ;
...
//	  printf ("clicou em %d, %d... ", x, y); 
	...
	case GLUT_RIGHT_BUTTON:  
	  ...
	  // atualiza as coordenadas de referencia para o zoom
	  window_to_world(x, y, zprReferencePoint);
	  /* 	  printf("coordenadas de mundo sao x=%f y=%f z=%f\n", zprReferencePoint[0], zprReferencePoint[1], */
	  /* 		 zprReferencePoint[2]); */
...
zprPick(GLdouble x, GLdouble y,GLdouble delX, GLdouble delY)
{
   ...
   /*          for (int k=0; k < (GLint) buffer[j]; k++) */
   ...
   if (hits)
   {
      for (i=0,j=0; i<hits; i++)
      {
         if (buffer[j+1]<(unsigned int)minZ)
         {
